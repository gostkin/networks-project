...
public:
    SingleGetMeTestCase() {
        Expectations = {"Client sends getMe request"};
    }
...
public:
    ErrorHandlingTestCase() {
        Expectations = {"Client sends getMe request and receives Internal Server error",
...
public:
    GetUpdatesAndSendMessagesTestCase() {
        Expectations = {"Client sends getUpdates request", "Client sends message \"Hi!\"",
...
public:
    HandleOffsetTestCase() {
        Expectations = {"Client sends request and receives 2 messages",
...
void FakeServer::Start() {
    ...
    Server_.reset(
        new HTTPServer(new FakeHandlerFactory(TestCase_.get()), *Socket_, new HTTPServerParams()));
